...
def gen_sample(src_dir,
    ...
    with open('{:s}/{}ing/{}.txt'.format(src_dir, phase, phase), 'w') as file:
        ...
        for image_name in os.listdir(ops.join(src_dir, b_gt_image_dir)):
            ...
            assert ops.exists(ops.join(src_dir,
                                       image_path)), '{:s} not exist'.format(
                                           ops.join(src_dir, image_path))
            assert ops.exists(ops.join(
                src_dir, instance_gt_image_path)), '{:s} not exist'.format(
                    ops.join(src_dir, instance_gt_image_path))

            b_gt_image = cv2.imread(
                ops.join(src_dir, binary_gt_image_path), cv2.IMREAD_COLOR)
            i_gt_image = cv2.imread(
                ops.join(src_dir, instance_gt_image_path), cv2.IMREAD_COLOR)
            image = cv2.imread(ops.join(src_dir, image_path), cv2.IMREAD_COLOR)
