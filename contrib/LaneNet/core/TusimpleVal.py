...
class TusimpleEval:
    ...
    def evaluate_pred(self, dataset, seg_pred, exist_pred, batch):
        ...
        for b in range(len(seg_pred)):
            seg = seg_pred[b]
            exist = [1 if exist_pred[b, i] > 0.5 else 0 for i in range(7 - 1)]
    ...
    def evaluate(self, dataset, output, batch):
        seg_pred, exist_pred = output[0], output[
            1]  # output['seg'], output['exist']
    ...
    def summarize(self):
        ...
        eval_result, acc = LaneEval.bench_one_submit(
            output_file,
